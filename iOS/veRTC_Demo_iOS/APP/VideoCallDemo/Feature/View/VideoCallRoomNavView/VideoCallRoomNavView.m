...
#import "GCDTimer.h"
@interface VideoCallRoomNavView ()
...
@implementation VideoCallRoomNavView
...
- (void)hangeupButtonAction:(UIButton *)sender {
    if ([self.delegate respondsToSelector:@selector(VideoCallRoomNavView:didSelectStatus:)]) {
        [self.delegate VideoCallRoomNavView:self didSelectStatus:RoomNavStatusHangeup];
    }
}

- (void)switchCameraBtn:(UIButton *)sender {
    if ([self.delegate respondsToSelector:@selector(VideoCallRoomNavView:didSelectStatus:)]) {
        [self.delegate VideoCallRoomNavView:self didSelectStatus:RoomNavStatusSwitchCamera];
    }
}
...
- (void)setLocalVideoSession:(VideoCallRoomUserModel *)localVideoSession {
    _localVideoSession = localVideoSession;
    NSString *roomIDStr = [localVideoSession.roomId stringByReplacingOccurrencesOfString:@"call_" withString:@""];
    if (roomIDStr.length > 6) {
...
- (BaseButton *)switchCameraBtn {
    if (!_switchCameraBtn) {
        ...
        [_switchCameraBtn setImage:[UIImage imageNamed:@"room_camera" bundleName:HomeBundleName] forState:UIControlStateNormal];
...
- (BaseButton *)hangeupButton {
    if (!_hangeupButton) {
        ...
        [_hangeupButton setImage:[UIImage imageNamed:@"room_hangeup" bundleName:HomeBundleName] forState:UIControlStateNormal];
