...
- (void)viewWillAppear:(BOOL)animated {
    ...
    // Reset the button state every time you return to the preview page
    [[VideoCallRTCManager shareRtc] updateRtcVideoParams];
    [[VideoCallRTCManager shareRtc] startPreview:self.videoView];
    [[VideoCallRTCManager shareRtc] setEnableSpeakerphone:ByteRTCAudioRouteSpeakerphone];
...
- (void)applicationBecomeActive {
    ...
    if ([topVC isKindOfClass:[VideoCallLoginViewController class]]) {
        ...
        alertModel.title = @"确定";
        [[AlertActionManager shareAlertActionManager] showWithMessage:@"本产品仅用于功能体验，单次会议时长不超过15分钟"
                                                              actions:@[alertModel]];
    }
}
...
- (void)onClickEnterRoom:(UIButton *)sender {
    ...
    [self.enterRoomBtn setTitle:@"加入房间中..." forState:UIControlStateNormal];
    [VideoCallRTMManager joinRoom:userModel
                            block:^(NSString * _Nonnull token,
                                    NSInteger duration,
                                    RTMACKModel * _Nonnull model) {
        [wself.enterRoomBtn setTitle:@"进入房间" forState:UIControlStateNormal];
        if (model.result) {
...
- (void)onClickEnableVideo:(BaseButton *)sender {
    if (sender.status != ButtonStatusIllegal) {
        ...
        [[VideoCallRTCManager shareRtc] enableLocalVideo:isEnableVideo];
    } else {
        AlertActionModel *alertCancelModel = [[AlertActionModel alloc] init];
        alertCancelModel.title = @"取消";
        AlertActionModel *alertModel = [[AlertActionModel alloc] init];
...
- (UITextField *)roomIdTextField {
    if (!_roomIdTextField) {
        ...
        NSAttributedString *attrString = [[NSAttributedString alloc] initWithString:@"请输入房间号，仅限1~18位纯数字"
                                                                         attributes:
