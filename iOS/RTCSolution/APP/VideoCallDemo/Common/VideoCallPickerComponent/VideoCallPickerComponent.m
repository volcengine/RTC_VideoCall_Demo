...
@interface VideoCallPickerComponent () <UIPickerViewDelegate,UIPickerViewDataSource>
...
@implementation VideoCallPickerComponent
    ...
    if (item) {
        ...
        if ([item isKindOfClass:[NSDictionary class]]) {
            for (int i = 0; i < dataLists.count; i++) {
                NSDictionary *data = dataLists[i];
                NSString *title = data[@"title"];
                NSString *titleItem = item[@"title"];
                if ([title isEqualToString:titleItem]) {
                    row = i;
                    break;
                }
            }
...
- (UIButton *)confirmButton {
    if (!_confirmButton) {
        ...
        [_confirmButton setTitle:LocalizedString(@"ok") forState:UIControlStateNormal];
