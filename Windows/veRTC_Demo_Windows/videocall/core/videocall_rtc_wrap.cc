		...
		[=](bytertc::LocalAudioStreamState state,
			bytertc::LocalAudioStreamError error) {
				if (state == bytertc::kLocalAudioStreamStateFailed &&
					...
					vrd::util::showToastInfo("麦克风打开失败，请检查设备");
			...
			bytertc::LocalVideoStreamError error) {
				if (state == bytertc::kLocalVideoStreamStateFailed &&
					...
					if (!videocall::DataMgr::instance().mute_video()) {
                        ...
                        vrd::util::showToastInfo("摄像头打开失败，请检查设备");
...
void VideoCallRtcEngineWrap::onAudioStateChanged(std::string device_id,
	...
	switch (device_state) {
	case bytertc::kMediaDeviceStateAdded:  // Insert a new device
		vrd::util::showToastInfo("有新麦克风设备插入");
		emit instance().sigUpdateAudioDevices();
		break;
	case bytertc::kMediaDeviceStateRemoved:  // Pull out the device
		vrd::util::showToastInfo("麦克风设备被拔出");
		emit instance().sigUpdateAudioDevices();
		break;
	default:
		break;
